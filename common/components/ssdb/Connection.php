...
namespace common\components\ssdb;

use common\components\ssdb\SsdbException as Exception;
use yii\base\Component;

/**
 * Class Connection for ssdb.
 *
 * @method info()
 * @method dbsize()
 * @method ping()
...
final class Connection extends Component
{
    ...
    {
        parent::init();
        $timeout_f = (float) $this->timeout / 1000;
        $this->sock = @stream_socket_client("$this->host:$this->port", $errno, $errstr, $timeout_f);
        if (!$this->sock) {
            throw new Exception("$errno: $errstr");
        }
        $timeout_sec = intval($this->timeout / 1000);
        $timeout_usec = ($this->timeout - $timeout_sec * 1000) * 1000;
    ...
    private function check_easy_resp($cmd, Response $resp)
    {
        $this->last_resp = $resp;
        if ($this->easy) {
            if ($resp->not_found()) {
